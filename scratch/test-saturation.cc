...
// TRAFIC DE SATURATION : OnOffApplication des STA vers AP1
for (uint32_t i = 0; i < wifiStaNodes.GetN(); ++i) {
    // Débit élevé (100 Mbps), envoi continu
    OnOffHelper onoff("ns3::UdpSocketFactory", InetSocketAddress(apInterfaces.GetAddress(0), 8000 + i));
    onoff.SetAttribute("DataRate", StringValue("1Mbps"));
    onoff.SetAttribute("PacketSize", UintegerValue(1472)); // MTU typique sans IP/TCP overhead
    onoff.SetAttribute("StartTime", TimeValue(Seconds(2.0)));
    onoff.SetAttribute("StopTime", TimeValue(Seconds(60.0)));
    onoff.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
    onoff.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]")); // pas de pause

    ApplicationContainer onOffApp = onoff.Install(wifiStaNodes.Get(i));
}

// Conteneur global pour tous les PacketSink installés
std::vector<Ptr<PacketSink>> packetSinks;


// Installation des PacketSink sur AP1, un par flux
for (uint32_t i = 0; i < wifiStaNodes.GetN(); ++i) {
    PacketSinkHelper sink("ns3::UdpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), 8000 + i));
    ApplicationContainer sinkApp = sink.Install(wifiApNodes.Get(0));
    sinkApp.Start(Seconds(1.0));
    sinkApp.Stop(Seconds(60.0));
    // Récupérer le Ptr<PacketSink> pour pouvoir lire les stats plus tard
    Ptr<PacketSink> sinkPtr = DynamicCast<PacketSink>(sinkApp.Get(0));
    packetSinks.push_back(sinkPtr);
}
...
//  Lancement de la simulation
    ...
    // Afficher le total de bytes reçus pour chaque PacketSink
    for (uint32_t i = 0; i < packetSinks.size(); ++i) {
        std::cout << "PacketSink " << i 
                << " a reçu " << packetSinks[i]->GetTotalRx() 
                << " bits au total, soit " << packetSinks[i]->GetTotalRx()/1000000.000000
                << " Mb " << std::endl;
    }
